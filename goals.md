...
@section('content')
    ...
    <form action="{{ route('goals.store') }}" method="POST">
        ...
        action = กดบันทึกแล้วส่งไปไหน (ไป /goals = method store)
        method="POST" = ส่งข้อมูลแบบซ่อน (ไม่โชว์บน URL)
        ------------------------------------------------------------------ --}}

        @csrf
        {{-- รหัสความปลอดภัย - ต้องมีทุกฟอร์ม! --}}

        <div class="form-group">
            <label>รหัสประตู</label>
            <input type="text" name="goal_id" class="form-control" placeholder="เช่น G001">
            {{-- ------------------------------------------------------------------
            name="goal_id" = ชื่อช่องนี้
            เมื่อกดบันทึก Controller จะเอาค่านี้ได้จาก $request->goal_id
            ------------------------------------------------------------------ --}}
        </div>
        ...
        <div class="form-group">
            ...
            <select name="match_id" class="form-control">
                ...
                {{-- name="match_id" = ค่าที่เลือกจะส่งไปใน $request->match_id --}}

                @foreach ($matches as $match)
                    {{-- วนสร้าง <option> ทีละแมทช์ --}}
                    {{-- $matches = รายการแมทช์ที่ Controller ส่งมา (จาก create()) --}}
                    <option value="{{ $match->match_id }}">
                        {{-- value = ค่าที่ส่งไป Controller (รหัส) --}}
                        {{-- ข้อความข้างใน = ที่คนเห็นบนหน้าจอ --}}
        ...
        {{-- ปุ่มยกเลิก กลับหน้ารายการ --}}

        <button type="submit" class="btn btn-primary">บันทึก</button>
        {{-- ------------------------------------------------------------------
        type="submit" = เมื่อกด จะส่งฟอร์มไป action ที่กำหนด
        btn btn-primary = ปุ่มสีน้ำเงิน
        ------------------------------------------------------------------ --}}
